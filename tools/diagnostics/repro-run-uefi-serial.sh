...
mkdir -p "${OUT_DIR}"
ts="$(date +%Y%m%d-%H%M%S)"
log_file="${OUT_DIR}/repro-uefi-serial-${ts}.log"

echo "[uefi-repro] root: ${ROOT_DIR}"
echo "[uefi-repro] timeout: ${TIMEOUT_SEC}s"
echo "[uefi-repro] mode: prompt-detected injection"
echo "[uefi-repro] command: ${REPRO_CMD}"
echo "[uefi-repro] log: ${log_file}"
    ...
    while True:
        ...
        if proc.stdout in ready:
            ...
            if not sent_cmd and any(marker in tail for marker in prompt_markers):
                try:
                    ...
                    print(f"[uefi-repro] injected command: {repro_cmd}")
        ...
        if sent_cmd and not sent_quit and cmd_sent_at is not None and (time.time() - cmd_sent_at) >= post_cmd_wait_sec:
            try:
                ...
                proc.stdin.flush()
                sent_quit = True
                print("[uefi-repro] sent QEMU quit sequence")
            except BrokenPipeError:
                pass
...
if [ "${status}" -eq 124 ]; then
  echo "[uefi-repro] timed out after ${TIMEOUT_SEC}s"
elif [ "${status}" -ne 0 ]; then
  echo "[uefi-repro] run exited with status ${status}"
else
  echo "[uefi-repro] run completed"
fi

echo "[uefi-repro] saved: ${log_file}"
echo "${log_file}"
